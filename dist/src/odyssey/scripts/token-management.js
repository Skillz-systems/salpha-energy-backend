...
exports.OdysseyTokenCLI = void 0;
...
class OdysseyTokenCLI {
    ...
    async generateToken(clientName, expirationDays = 365) {
        try {
            ...
            console.log('');
            console.log('⚠️  Important: Store this token securely. It cannot be retrieved again.');
        }
        catch (error) {
    ...
    async listTokens() {
        try {
            ...
            tokens.forEach((token, index) => {
                ...
                const daysUntilExpiry = Math.ceil((token.expiresAt.getTime() - new Date().getTime()) /
                    (1000 * 60 * 60 * 24));
                ...
                console.log(`   Expires: ${token.expiresAt.toLocaleDateString()} (${daysUntilExpiry} days)`);
                console.log(`   Last Used: ${token.lastUsedAt ? token.lastUsedAt.toLocaleDateString() : 'Never'}`);
                console.log('');
            });
        }
        catch (error) {
...
exports.OdysseyTokenCLI = OdysseyTokenCLI;
async function main() {
