...
describe('PermissionsController', () => {
  ...
  describe('create', () => {
    it('should create a new permission', async () => {
      const createPermissionDto: CreatePermissionDto = { action: 'read', subject: 'all' };
  ...
  describe('findOne', () => {
    it('should return a permission by ID', async () => {
      ...
      expect(mockPermissionsService.findOne).toHaveBeenCalledWith('66f4237486d300545d3b1f10');
    });
  
    it('should throw NotFoundException if permission does not exist', async () => {
      // Here we reject the promise with an actual error
      mockPermissionsService.findOne.mockRejectedValue(new NotFoundException('Permission not found'));
  ...
  describe('update', () => {
    it('should update a permission', async () => {
      const updatePermissionDto: UpdatePermissionDto = { action: 'write', subject: 'all' };
      const updatedPermission = { id: '66f4237486d300545d3b1f10', ...updatePermissionDto };
      ...
      const result = await controller.update('66f4237486d300545d3b1f10', updatePermissionDto);
  
      expect(result).toEqual({
        message: `Permission with ID 66f4237486d300545d3b1f10 updated successfully`,
        data: updatedPermission,
      });
      expect(mockPermissionsService.update).toHaveBeenCalledWith('66f4237486d300545d3b1f10', updatePermissionDto);
    });
    ...
    it('should throw NotFoundException if permission does not exist', async () => {
      ...
      mockPermissionsService.update.mockRejectedValue(new NotFoundException('Permission not found'));
  
      await expect(controller.update('66f42a3166aaf6fbb2a643bf', updatePermissionDto)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('remove', () => {
    it('should remove a permission', async () => {
      mockPermissionsService.remove.mockResolvedValue({ message: "Permission deleted successfully" });
      ...
      expect(mockPermissionsService.remove).toHaveBeenCalledWith('66f4237486d300545d3b1f10');
    });
  
    it('should throw NotFoundException if permission does not exist', async () => {
      mockPermissionsService.remove.mockRejectedValue(new NotFoundException('Permission not found'));
